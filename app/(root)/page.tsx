  ...
  return (
    <>
      ...
      <section className="section_container">
        <p className="text-30-semibold text-black">{query ? `Search results for "${query}"` : "All Startups"}</p>
        <ul className="mt-7 card_grid">
          {posts.length > 0 ? posts.map((post: StartupTypeCard) => <StartupCard key={post?._id} post={post} />) : null}
        </ul>
      </section>
      <SanityLive />
    </>
